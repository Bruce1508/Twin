    ...
    <div className="min-h-screen bg-paper">
      <div className="max-w-3xl mx-auto px-4 py-12 space-y-8">
        ...
        {stage.name === "setup" && (
          <>
            <header>
              <p className="text-xs text-ink-faint uppercase tracking-widest mb-1">Compréhension écrite</p>
              <h1 className="text-2xl font-semibold text-ink">Lire et comprendre</h1>
              <p className="mt-1 text-sm text-ink-muted">5 questions de style TCF Canada sur un texte B2.</p>
            </header>

            <div className="flex gap-1 border-b border-rule">
              {(["generate", "paste"] as const).map((t) => (
                <button key={t} onClick={() => setTab(t)}
                  className={`px-4 py-2 text-sm font-medium border-b-2 transition-colors ${tab === t ? "border-zinc-900 dark:border-zinc-100 text-ink" : "border-transparent text-ink-faint"}`}>
            ...
            <form onSubmit={handleGenerate} className="space-y-4">
              {tab === "generate" ? (
                <div className="flex gap-3">
                  ...
                  <div>
                    <label className="block text-sm font-medium text-ink-muted mb-1">Registre</label>
                ...
                <div>
                  <label className="block text-sm font-medium text-ink-muted mb-1">Colle ton texte en français</label>
        ...
        {stage.name === "answering" && (
          <>
            <header>
              <p className="text-xs text-ink-faint uppercase tracking-widest mb-1">Compréhension écrite</p>
              <h1 className="text-xl font-semibold text-ink">Réponds aux questions</h1>
              <p className="text-sm text-ink-muted mt-1">Relie-toi au texte pour chaque réponse. Réponds en français.</p>
            </header>

            <div className="rounded-none border border-rule bg-paper-raised px-5 py-4 text-sm text-ink-muted leading-relaxed whitespace-pre-wrap">
              {stage.articleText}
            </div>
              ...
              {stage.questions.map((q, i) => (
                <div key={q.id} className="space-y-2">
                  ...
                  <textarea value={answers[q.id] ?? ""}
                    ...
                    className="w-full rounded-none border border-rule bg-paper-raised px-3 py-2 text-sm resize-none ml-5" />
                </div>
              ))}
          ...
          <>
            ...
            <div className="space-y-4">
              {stage.results.map((r, i) => (
                <div key={r.question_id} className={`rounded-none border p-4 space-y-2 ${QUALITY_STYLE[r.correction_quality]}`}>
                  <div className="flex items-start gap-2">
                  ...
                  <div className="ml-4 space-y-1">
                    ...
                    <p className="text-xs text-ink-faint italic">« {r.relevant_passage} »</p>
                  </div>
                </div>
              ))}
            </div>
